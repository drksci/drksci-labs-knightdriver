 ...
 *    ```
 */
let ShowPinsElement = class ShowPinsElement extends lit_element_1.LitElement {
    constructor() {
        super(...arguments);
        this.pinColor = 'red';
    }
    ...
    render() {
        ...
        const { pinColor } = this;
        return lit_element_1.html `<div style="position: relative">
      ...
      <svg style="position: absolute; top: 0; left: 0" width="100%" height="100%" fill=${pinColor}>
        ${pinInfo.map((pin) => lit_element_1.svg `<circle cx=${pin.x} cy=${pin.y} r=2><title>${pin.name}</title></circle>`)}
      </svg>
    </div>`;
    }
};
...
__decorate([
    lit_element_1.query('#content')
], ShowPinsElement.prototype, "elementSlot", void 0);
ShowPinsElement = __decorate([
    lit_element_1.customElement('wokwi-show-pins')
], ShowPinsElement);
